...
import fbLogger from '../../logger.mjs';
import getUser from '../api/getUser.mjs';
import userIp from '../../../http/userIP.mjs';
import getDomainURL from '../../../http/getDomainURL.mjs';
import login from '../firebase_redirect/login.mjs';
import logout from '../firebase_redirect/logout.mjs';
import addGuildMember from '../api/addGuildMember.mjs';
import getGuildWidget from '../api/getGuildWidget.mjs';
import getUserConnections from '../api/getUserConnections.mjs';
import discordAuth from '../index.mjs';
import getUserGuilds from '../api/getUserGuilds.mjs';
import getCookieSession from '../get/cookie-session.mjs';
import getFirebaseCookieSession from '../../../socket.io/cookie-session.mjs';
...
export default function cookieSession(app, cfg) {
  ...
  if (objType(cfg, 'object')) {
    ...
    const sessionFirebaseDiscordValidator = function (req) {
      return (
        req.firebase_session.uid ===
        `${discordSession.varsTemplate.uid}${tinyAuth.client_id}_${req.discord_session.user.id}`
      );
    };
    ...
    // Logger
    const logger = fbLogger;

    // Get Discord User
    const getDiscordUser = getUser;
    ...
    discordSession.firebaseAuth.redirect = {
      ...
      login: function (req, res, redirect_url, csrfToken) {
        return discordSession.firebaseAuth.redirect.template(
          'firebaseLogin',
          req,
          res,
          redirect_url,
    ...
    discordSession.set = function (req, tokenRequest, extra_text = '') {
      ...
      // Expire In
      req.session[sessionVars.token_expires_in + extra_text] = moment
        .tz('Universal')
        .add(tokenRequest.expires_in, 'second')
        .format();
    };
    ...
    const prepare_fire_auth_discord = function (req, user) {
      ...
      // Scope
      if (typeof dsSession.scope === 'string') preparedsSession.scope = dsSession.scope.split(' ');
      else preparedsSession.scope = dsSession.scope;
    ...
    discordSession.firebase.updateUser = function (access_token, user, oldUser) {
      return new Promise(function (resolve, reject) {
        ...
        if (validator.user !== validator.oldUser) {
          ...
          const firebaseAccount = discordSession.firebase.createAccountData(
            access_token,
            user,
            oldUser,
          );
    ...
    discordSession.firebase.get = function (req, res) {
      return new Promise(function (resolve, reject) {
        // Prepare Auth
        cfg.firebase.auth
          .verifySessionCookie(req.session[sessionVars.firebase_token])
          ...
          .then((decodedToken) => {
            ...
            if (tinyCfg.ipVerification) {
              ...
              // Convert
              if (typeof decodedToken.user_ip === 'string')
                decodedToken.user_ip = [decodedToken.user_ip];

              if (typeof requestIpAddress.value === 'string')
                requestIpAddress.value = [requestIpAddress.value];

              // Verification
              if (
                hash(decodedToken.user_ip) === hash(requestIpAddress.value) &&
                decodedToken.user_ip_type === requestIpAddress.type
              ) {
              ...
              // Nope
              else reject({ code: 406, message: 'Invalid User IP Session!' });
            }
    ...
    // Firebase Redirect
    const firebase_redirect = { login, logout };
    ...
    const final_functions = {
      ...
      sessionValidator: function (data = {}) {
        return (req, res, next) => {
          ...
          const validatorItems = {
            values: ['getUserConnections', 'getUser', 'getUserGuilds'],
            count: 0,
            callback: function () {
              // The Item
              ...
              // Next
                ...
                final_functions.sessionPlugins.validator(req, res, () => {
                  next();
                });
            },
          };
      ...
      sessionPlugins: {
        ...
        validator: function (req, res, next) {
          ...
          // Preparing Clocks
          if (!req.utc_clock) req.utc_clock = { now: moment.tz('Universal') };
          ...
          const existDiscordSession =
            typeof req.session[sessionVars.token_expires_in] === 'string' &&
            typeof req.session[sessionVars.access_token] === 'string' &&
            typeof req.session[sessionVars.refresh_token] === 'string';

          // Exist Session
          if (
            existDiscordSession &&
            !req.url.startsWith(tinyURLPath.firebaseLogin + '?') &&
            req.url !== tinyURLPath.firebaseLogin &&
            !req.url.startsWith(tinyURLPath.firebaseLogout + '?') &&
            req.url !== tinyURLPath.firebaseLogout
          ) {
            // Exist Firebase
            if (cfg.firebase) {
              // Finish the Session
              ...
              discordSession.firebase
                ...
                .then(() => {
                  ...
                  // Nope
                  else finishTinySession();
                })
                .catch(finishTinySession);
            }

            // Nope
            else checkDiscordSession(req, res, next);
          } else next();
        },
        ...
        getUserConnections: function (req, res, next) {
          ...
          if (typeof req.session[sessionVars.access_token] === 'string') {
            getUserConnections(req.session[sessionVars.access_token])
              .then((connections) => {
                // Set Discord Data
        ...
        getUserGuilds: function (req, res, next) {
          // Get User Discord Data
          if (typeof req.session[sessionVars.access_token] === 'string') {
            getUserGuilds(req.session[sessionVars.access_token])
              .then((guilds) => {
                // Set Discord Data
    ...
    const getSessionFromCookie = getCookieSession;
    ...
    const logout_firebase = (res, req, redirect, firebase_auth) => {
      // Firebase Logout
      if (cfg.firebase)
        return discordSession.firebaseAuth.redirect.logout(
          req,
          res,
          redirect,
          req.csrfToken.now.value,
          firebase_auth,
        );
      // Normal
      else {
    ...
    const logout_action = function (req, csrfToken, redirect) {
      return new Promise(function (resolve) {
        ...
        // Normal
        if (typeof req.session[sessionVars.access_token + '_command'] !== 'string') finalResult();
    ...
    const auto_logout = function (req, err) {
      return new Promise(function (resolve, reject) {
        ...
        logout_action(req, req.csrfToken.now.value, req.url).then((result) => {
          ...
          logout_result(req, result.user).then(resolve);
        });
      });
    };
    ...
    const checkDiscordSession = function (req, res, next) {
      ...
      const restChecker = function () {
        // Add to Req
        if (
          typeof req.session[sessionVars.access_token + '_command'] === 'string' ||
          typeof req.session[sessionVars.access_token + '_command'] === 'number'
        ) {
          // Add Access Command
          req.discord_session.commands = {};
          req.discord_session.commands.access_token =
            req.session[sessionVars.access_token + '_command'];

          // Other Values
          if (
            typeof req.session[sessionVars.token_type + '_command'] === 'string' ||
            typeof req.session[sessionVars.token_type + '_command'] === 'number'
          ) {
            req.discord_session.commands.token_type =
              req.session[sessionVars.token_type + '_command'];
          }

          if (
            typeof req.session[sessionVars.scope + '_command'] === 'string' ||
            typeof req.session[sessionVars.scope + '_command'] === 'number'
          )
        ...
        if (req.utc_clock.ds_token_time_left < 1440) {
                  ...
                  if (cfg.firebase) {
                    ...
                    getDiscordUser(access_token)
                      .then((user) => {
                        ...
                        cfg.firebase.auth
                          .setCustomUserClaims(
                            discordSession.uidGenerator(user.id),
                            prepare_fire_auth_discord(req, user),
                          )
                          .then(() => {
                            // Update User Data
          ...
          if (cfg.firebase && req.discord_session.user && objType(req.firebase_session, 'object')) {
            ...
            // Nope
              ...
              cfg.firebase.auth
                .setCustomUserClaims(
                  discordSession.uidGenerator(req.discord_session.user.id),
                  prepare_fire_auth_discord(req, req.discord_session.user),
                )
                .then(() => make_the_update())
                .catch((err) => prepare_final_session(req, res, err));
          }
      ...
      req.utc_clock.ds_token_expires_in = moment.tz(
        req.session[sessionVars.token_expires_in],
        'Universal',
      );

      // Time Left
      req.utc_clock.ds_token_time_left = req.utc_clock.ds_token_expires_in.diff(
        req.utc_clock.now,
        'minutes',
      );

      // Command Time Left
      if (!req.session[sessionVars.token_expires_in + '_command']) return restChecker();
      // Nope
      else {
        // Get Token Expiration
        req.utc_clock.ds_token_expires_in_command = moment.tz(
          req.session[sessionVars.token_expires_in + '_command'],
          'Universal',
        );

        // Time Left
        req.utc_clock.ds_token_time_left_command = req.utc_clock.ds_token_expires_in.diff(
          req.utc_clock.now,
          'minutes',
        );
        ...
        if (req.utc_clock.ds_token_time_left_command < 1440) {
          ...
          const logoutCommand = function () {
            // Revoke Token
            revokeToken(req.session[sessionVars.access_token + '_command'], commandAuth)
              .then((data) => {
          ...
          if (req.utc_clock.ds_token_time_left_command > 0) {
            discordAuth
              ...
              .then((result) => {
                // Complete
                if (result.refreshed) discordSession.set(req, result.tokenRequest, '_command');
      ...
      if (req.discord_session.user) {
        ...
        if (objType(req.body, 'object') && typeof req.body.token === 'string') {
          ...
          if (
            ...
            req.csrfToken.now.value.length < 1 ||
            (typeof req.body.csrfToken === 'string' &&
              req.csrfToken.now.value === req.body.csrfToken)
          ) {
            // Prepare Module
            const newCookie = new AuthSystem();
            ...
            // Action
            newCookie
              .genCookieSession(cfg.firebase.auth, req.body.token)
              .then((sessionCookie) => {
          ...
          // Nope
          else return res.json({ success: false, error: 'Invalid csrfToken!' });
        }

        // Nope
        else return res.json({ success: false, error: 'Invalid Data!' });
      }

      // Nope
      else return res.json({ success: false, error: 'Invalid Discord User!' });
    };

    if (cfg.bodyParser)
      app.post(
        tinyURLPath.firebaseLogin,
        bodyParser.json,
        bodyParser.urlencoded,
        final_functions.sessionPlugins.getUser,
        final_functions.sessionPlugins.validator,
    ...
    app.get(tinyURLPath.firebaseLogin, final_functions.sessionPlugins.validator, (req, res) => {
      ...
      // Exist Redirect
      if (typeof req.query.redirect === 'string') final_data.redirect = '/' + req.query.redirect;
      else final_data.redirect = '/';

      // Auth Token
      if (typeof req.session[sessionVars.firebase_auth_token] === 'string') {
        // Insert Data
        if (typeof req.query.key === 'string' && req.query.key.length > 0) {
          final_data.key = req.query.key;
        }
    ...
    if (cfg.bodyParser)
      app.post(
        tinyURLPath.firebaseLogout,
        bodyParser.json,
        bodyParser.urlencoded,
        final_functions.sessionPlugins.validator,
        firebaseLogoutCallback,
      );
    else
      app.post(
        tinyURLPath.firebaseLogout,
    ...
    if (typeof tinyURLPath.botLogin === 'string') {
      app.get(tinyURLPath.botLogin, (req, res) => {
        return res.redirect(
          `${config.oauth2}?client_id=${encodeURIComponent(tinyAuth.client_id)}&scope=${encodeURIComponent(botAuth.discordScope.join(' '))}&permissions=${botAuth.permissions}`,
        );
      });
    }
        ...
        .then((result) => {
          ...
          if (result.state.type === 'login' && result.newSession) {
            ...
            if (!tinyCfg.needEmailVerified || result.user.verified) {
              ...
              if (cfg.firebase) {
                discordSession.firebase
                  .set(req, result.user)
                  .then(() => {
                    discordSession.firebaseAuth.redirect.login(
                      req,
                      res,
                      result.redirect,
                      result.state.csrfToken,
                    );
                  })
                  .catch((err) => {
                    // Firebase Auth
                    ...
                    // Logout
                    auto_logout(req, { code: 500, message: err.message })
                      .then((result) => {
                        logout_firebase(res, req, result.redirect, firebase_auth);
                      })
                      .catch((err) => {
            ...
            // Nope
              ...
              tinyCfg.errorCallback(
                { code: 401, message: 'This email is not a verified email!' },
                req,
                res,
              );
          }
  ...
  // Nope
  else throw new Error('Invalid Config Value!');
}
